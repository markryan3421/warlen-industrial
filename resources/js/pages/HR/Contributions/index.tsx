...
const route = (name: string, params?: any) => {
    ...
    const urls: Record<string, string> = {
        'hr.contribution-versions.index': '/hr/contributions',
        'hr.contribution-versions.store': '/hr/contributions',
        'hr.contribution-versions.update': `/hr/contributions/${params?.contribution_version || ''}`,
        'hr.contribution-versions.destroy': `/hr/contributions/${params?.contribution_version || ''}`,
    };
    return urls[name] || '/hr/contributions';
};

const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Contributions', href: '/hr/contributions' },
];
...
export default function Index({
    ...
    const confirmDelete = () => {
        ...
        setIsDeleting(true);
        destroy(route('hr.contribution-versions.destroy', { contribution_version: itemToDelete.id }), {
            onSuccess: () => {
    ...
    const fetchEmployeesAndSettings = async () => {
        ...
        try {
            const employeesResponse = await fetch('/hr/employees/list');
            ...
            const settingsPromises = versions.map(version =>
                fetch(`/hr/employee-contribution-settings?contribution_version_id=${version.id}`)
                    .then(res => {
    ...
    const saveContributionSettings = (contributionVersionId: number) => {
        ...
        fetch('/hr/employee-contribution-settings/bulk', {
            method: 'POST',
            headers: {
                                ...
                                {displayData.length > 0 && contributionVersions && (
                                    <CustomPagination
                                        ...
                                        onPerPageChange={(value) => {
                                            router.get(
                                                route('hr.contribution-versions.index'),
...
function CreateContributionModal({ isOpen, onClose, existingTypes }: CreateContributionModalProps) {
    ...
    function submitContributionVersion(e: React.FormEvent) {
        e.preventDefault();
        post(route('hr.contribution-versions.store'), {
            onSuccess: () => {
                handleClose();
            },
            onError: (errors) => {
...
function EditContributionModal({ isOpen, onClose, contributionVersion, existingTypes }: EditContributionModalProps) {
    ...
    function submitContributionVersion(e: React.FormEvent) {
        e.preventDefault();
        put(route('hr.contribution-versions.update', { contribution_version: contributionVersion.id }), {
            onSuccess: () => {
                handleClose();
            },
            onError: (errors) => {
