...
export default function Index({ applicationLeaves }: ApplicationLeaveProps) {
    ...
    const getStatusBadgeClass = (status: string) => {
        ...
        switch (statusLower) {
            case 'approved':
                return 'bg-green-100 text-green-800 py-2.5 px-1';
            case 'rejected':
                return 'bg-red-100 text-red-800 py-2.5 px-1';
            case 'pending':
            default:
                return 'bg-yellow-100 text-yellow-800 py-2.5 px-1';
        }
    };
    ...
    const columns = [
        ...
        },
        {
            ...
            isBadge: true,
            render: (row: any) => (
                <span className={`inline-flex items-center rounded-full text-xs font-medium ${getStatusBadgeClass(row.app_status)}`}>
