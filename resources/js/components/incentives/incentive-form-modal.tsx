...
export function IncentiveFormModal({
    ...
    // Initialize edit form when incentive data is available
    useEffect(() => {
        if (isEditing && incentive && isOpen) {
            console.log('Initializing edit form with:', incentive);
            
            // Extract employee IDs from the employees array
            const employeeIds = incentive.employees?.map(emp => emp.id) || [];
            ...
            editForm.setData({
                ...
                payroll_period_id: String(incentive.payroll_period?.id || incentive.payroll_period_id || ''),
                employee_ids: employeeIds,
            });
        }
    }, [isEditing, incentive, isOpen]);
    ...
    useEffect(() => {
        if (isOpen) {
            console.log(`=== ${isEditing ? 'Edit' : 'Create'} Incentive Form Modal Debug ===`);
    ...
    }, [isOpen, isEditing, incentive, data, employees]);
    ...
    const submit = (e: React.FormEvent) => {
        ...
        const options = {
            onSuccess: () => {
                toast.success(isEditing ? 'Incentive updated successfully!' : 'Incentive created successfully!');
                onClose();
        ...
        if (isEditing && incentive) {
            editForm.put(`/incentives/${incentive.id}`, options);
        } else {
            createForm.post('/incentives', options);
        }
    };
