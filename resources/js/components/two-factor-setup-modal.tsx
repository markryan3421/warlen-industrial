...
function TwoFactorVerificationStep({
    onSuccess,   // ← renamed from onClose: parent handles what happens after confirm
    onBack,
}: {
    onSuccess: () => void;
    onBack: () => void;
    ...
    return (
        <Form
            {...confirm.form()}
            onSuccess={() => onSuccess()}  // ← calls parent's handleVerificationSuccess
            resetOnError
            resetOnSuccess
        >
            {({
...
type Props = {
    isOpen: boolean;
    onClose: () => void;
    onSuccess?: () => void;                    // ← NEW: called after full setup completes
    ...
    fetchRecoveryCodes: () => Promise<void>;   // ← NEW: fetches codes after confirmation
    errors: string[];
};
...
export default function TwoFactorSetupModal({
    ...
    /**
     * Called when the TOTP confirm form succeeds:
     * 1. Reload Inertia props so twoFactorEnabled flips to true in the UI
     * 2. Fetch recovery codes so they appear immediately in the Enabled view
    ...
    }, [clearSetupData, fetchRecoveryCodes, onClose, onSuccess]);
                ...
                <div className="flex flex-col items-center space-y-5">
                    {showVerificationStep ? (
                        <TwoFactorVerificationStep
                            onSuccess={handleVerificationSuccess}  // ← fixed
