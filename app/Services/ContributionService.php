...
class ContributionService
{
    ...
    private function calculateSSSContribution($version, float $grossPay, ?int $employeeId): array
    {
        ...
        // 1. Employee‑specific settings (exemption / fixed amount)
        if ($employeeId) {
        ...
        // 2. Special rule: gross pay >= 34,750 → employee_share = 1,750
        if ($grossPay >= 34750) {
            // Find the appropriate employer share from the bracket (or nearest bracket)
            ...
            if (!$bracket) {
                $bracket = $this->findNearestBracket($version, $grossPay);
            }

            $employerShare = $bracket ? (float) $bracket->employer_share : 0;
            ...
            Log::info("SSS Special Rule (>=₱34,750):", [
                'gross_pay' => $grossPay,
